        ...
        <>
                ...
                >
                  ...
                  <div className="calendar-events-preview">
                    {dayEvents.slice(0, 2).map((event) => (
                      <div
                        key={event.id}
                        className={`calendar-event-pill ${leaveTypeClassMap[event.leave_type] || "calendar-event-vacation"}`}
                      >
                        {event.employee_name}
                      </div>
                    ))}
                    ...
                    {dayEvents.length > 2 && (
                      <div className="calendar-more">+{dayEvents.length - 2} więcej</div>
                    )}
                  </div>
                </div>
              );
          ...
          {events.length === 0 && (
            <div className="empty-card">
              Brak zatwierdzonych nieobecności zespołu w tym miesiącu.
            </div>
          )}
        </>
      )}
        ...
        <div className="modal-overlay" onClick={closeDayModal}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            <div className="page-header">
              ...
              <button type="button" className="secondary-button" onClick={closeDayModal}>
                Zamknij
              </button>
            </div>
              ...
              {selectedDayEvents.map((event) => (
                <div key={event.id} className="calendar-modal-item">
                  ...
                  <div className="calendar-modal-meta">
                    ...
                    <div>
                      <strong>Komentarz decyzji:</strong> {event.decision_comment || "Brak"}
                    </div>
                  </div>
                </div>
              ))}
